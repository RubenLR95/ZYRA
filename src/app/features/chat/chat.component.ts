...
export class ChatComponent implements AfterViewChecked {
  ...
  constructor(private gemini: GeminiService) {} // Inyección del servicio Gemini. ESTO LLAMA A LA API

  // MÉTODO PARA INVOCAR ENVIAR UN MENSAJE
  send(): void {
    const text = this.userInput.trim(); // VARIABLE PARA EL MENSSAJE ENVIADO POR EL USUARIO . .trim para eliminar espacios al inicio y fin
    ...
    this.gemini.generate(text).subscribe({
      ...
      error: err => {       // En caso de error, mostrar mensaje de error en el chat
